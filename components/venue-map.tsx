...
export default function VenueMap() {
  ...
  // Google Maps URL for Bride's House
  const googleMapsUrl = "https://maps.app.goo.gl/r32qpHhSk5t2dc8d9"
    ...
    <section className="w-full px-4 sm:px-6 py-8">
      <div className="max-w-4xl mx-auto">
        <div className="relative group">
          ...
          <div className="relative w-full aspect-video min-h-[250px] rounded-2xl overflow-x-hidden border-2 border-foreground/10 shadow-2xl transition-all duration-300 transform hover:scale-[1.02] cursor-pointer bg-white">            <div
          ...
          <div className="mt-6 text-center px-4">
            <h3 className="text-xl sm:text-2xl font-serif font-medium text-foreground mb-2">
              {isRTL ? 'فيلا لا روز' : "Villa La Rose"}
            </h3>
            <div className="text-muted-foreground text-sm sm:text-base mb-3 space-y-1">
              {isRTL ? (
                <>
                  <div>كفر الشيخ</div>
                  <div>اسفل الكوبري العلوي</div>
                </>
              ) : (
                <>
                  <div>Kafr El Sheikh</div>
                  <div>under the upper bridge</div>
                </>
              )}
            </div>
            <div className="flex flex-col sm:flex-row justify-center items-center gap-3 sm:gap-4 text-xs sm:text-sm text-muted-foreground">
              <div className="flex items-center gap-2">
                ...
                <span>{isRTL ? 'كفر الشيخ، مصر' : 'Kafr El Sheikh, Egypt'}</span>
              </div>
              <div className="flex items-center gap-2">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
